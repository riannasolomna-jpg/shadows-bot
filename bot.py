...
def run_flask():
    ...
    # use_reloader=False предотвращает дублирование потоков во Flask
...
def send_smart_message(chat_id, text, reply_markup=None, parse_mode=None):
    """Безопасная отправка длинных сообщений с разбиением на фрагменты"""
    if len(text) <= 4000:
        try:
            bot.send_message(chat_id, text, parse_mode=parse_mode, reply_markup=reply_markup)
        except Exception:
            # Если Markdown упал из-за спецсимволов, отправляем обычным текстом
        ...
        for i, chunk in enumerate(chunks):
            ...
            try:
                bot.send_message(chat_id, chunk, parse_mode=parse_mode, reply_markup=m)
            except Exception:
...
def handle_role_selection(call):
    role = "Домовец" if call.data == "role_domovets" else "Персонал "
...
def handle_admin_actions(call):
    ...
    if action == "accept":
        try:
            ...
            bot.send_message(ADMIN_ID, f"Ошибка при принятии: {e}")
...
def process_buffered_application(user_id, message_obj):
    ...
    if "Требует правок" in report:
        user_response = (
            ...
            f"{report}\n\n"
            "📌 Пожалуйста, исправьте указанные недочёты и отправьте анкету повторно."
        )
        send_smart_message(message_obj.chat.id, user_response, parse_mode="Markdown")
    else:
...
def handle_all_messages(message):
    # 1. Обработка ответа администратора игроку
    ...
    # Игнорируем сервисные команды, если они случайно сюда попали
    if message.text.startswith('/'):
        return

    # 2. Сбор текста анкеты от пользователя
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            print(f"Сбой в polling: {e}. Перезапуск через 5 секунд...")
            time.sleep(5)
